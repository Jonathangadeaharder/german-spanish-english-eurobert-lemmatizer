"""Generate b2_new_003.conllu (es_b2_train_401–600) with Stanza + validation."""
...
from b2_new_003_sentences import SENTENCE_BATCHES, SENTENCES
...
SER_FORMS = {
    ...
    "eres", "somos", "sois", "fui", "fuimos", "fuisteis",
}
ESTAR_FORMS = {
...
def normalize_token(form: str, upos: str, lemma: str) -> tuple[str, str]:
    ...
    if upos == "VERB":
        ...
        if participle is not None:
            lemma = participle
        participle = _participle_to_infinitive(fl)
        if participle is not None:
            lemma = participle
        if not any(lemma.endswith(e) for e in ("ar", "er", "ir", "se", "ír")):
            # Stanza sometimes returns conjugated form as lemma
    ...
    if upos == "NOUN" and lemma:
        ...
        if lemma.endswith("es") and fl.endswith("es") and len(lemma) > 3:
            ...
            lemma = lemma[:-1]
        if fl.endswith("s") and not fl.endswith("ss") and fl[:-1] != lemma:
            singular = fl[:-1]
            if singular.startswith(lemma) and len(singular) > len(lemma):
                lemma = singular
...
def main() -> None:
    ...
    for i, sent in enumerate(SENTENCES, START_ID):
        ...
        if tc < MIN_TOKENS or tc > MAX_TOKENS:
            print(f"PRE-CHECK es_b2_train_{i:03d}: {tc} tokens — {sent}")
            sys.exit(1)
    ...
    target_path = project_root / "data/handcraft/es/train/b2_new_003.conllu"
    ...
    for batch_num, batch in enumerate(SENTENCE_BATCHES):
        ...
        for sent in batch:
            sent_id = f"es_b2_train_{START_ID + global_idx:03d}"
            doc = nlp(sent)
